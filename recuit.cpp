...
long double P(long double old, long double nouv, long double temp)
{
    //if (clock()-deb < 3e3) cout << 'a' << old << ' ' << nouv << ' ' << temp << ' ' << exp((old-nouv)/temp) << '\n';
    if (nouv < old) return 2.0;
    return exp((old-nouv)/temp);
}
...
int main()
{
    ...
    {
        long double temp = tD * pow(tF/tD, (clock()-deb)/1.95e6);
        ...
        if (P(actu, nouv, temp) > rand()/(long double)INT_MAX)
        {
            //if (clock()-deb < 3e3) cout << exp((actu - nouv)/temp) << ' ' << actu << ' ' << nouv << ' ' << P(actu, nouv, temp) << ' ' << rand()/(long double)INT_MAX << '\n';
            actu= nouv;
            cur = voisin;
        }
        /*
